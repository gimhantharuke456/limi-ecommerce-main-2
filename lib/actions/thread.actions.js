...
export async function fetchPosts(pageNumber = 1, pageSize = 20) {
  ...
  const postsWithAuthorDetails = await Promise.all(
    posts.map(async (post) => {
      ...
      return { ...post._doc, author: authorDetails };
    })
  );
...
export async function fetchThreadById(threadId) {
  try {
    ...
    const populateAuthorDetails = (thread) => {
      if (thread.children) {
        thread.children = thread.children.map((child) => {
          child.author = author; // Assign author details to child threads
          return populateAuthorDetails(child);
        });
      }
      return thread;
    };

    return populateAuthorDetails({
      ...thread.toObject(), // Convert Mongoose document to plain object
      author: author,
    });
  } catch (err) {
    console.error("Error while fetching thread:", err);
    throw new Error("Unable to fetch thread" + err);
  }
}
